    ...
    async def teamtrees(self, ctx):
      ...
      for i in range(len(count)-1, -1, -1):
        ...
        if j == 3:
          ...
          j = 0
      count = ''.join(count)
      # await ctx.send(f'{count} Trees planted so far')
      ...
      for planted in trees_planted[:10]:
        finalres += f'\n {names[i].ljust(20)} planted: {planted}'
        i = i + 1
      finalres = ''.join(finalres)

      embed = discord.Embed(
                        title = f'{count} Trees planted so far',
                        colour = ctx.author.colour,
                        description = finalres
                        )
      await ctx.send(embed = embed)
